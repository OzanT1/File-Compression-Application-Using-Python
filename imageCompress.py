...
class LZWCoding:
    ...
    def write_compressed_file(self):
        ...
        with open(self.path, 'r+') as file, open(output_path, 'wb') as output, open(output_path_2, 'w+') as file2:
            ...
            elif self.dataType == "graylevelimage":
    ...
    def decompress_file(self,input_path):
        ...
        with open(input_path, 'rb') as file, open(output_path, 'w') as output:
            ...
            if self.dataType == "number":
                ...
                orgImageFile = Image.open(self.path)
                nrows = orgImageFile.size[1]
                ncols = orgImageFile.size[0]
                out = []
                for cc in decompressed_text:
                    out.append(ord(cc))
                output.write(",".join(str(num) for num in out))
                ...
                img_matrix = np.reshape(out, (nrows,ncols))   # this may cause error when the codelength is not enough. Try increasing it.
                img = Image.fromarray(np.uint8(img_matrix))
                img.show()
                ...
                img_matrix = np.reshape(out, (nrows, ncols, 3))  # this may cause error when the codelength is not enough. Try increasing it.
                img = Image.fromarray(np.uint8(img_matrix))
                img.show()
                img.save("{}_decompressed.bmp".format(filename))
